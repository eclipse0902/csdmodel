...
class EEGConnectivityModel(nn.Module):
    ...
    def compute_pretrain_loss(self, x: torch.Tensor, mask: torch.Tensor) -> Tuple[torch.Tensor, Dict]:
        ...
        # Forward pass (±âÁ¸°ú µ¿ÀÏ)
        reconstructed = self.forward(masked_x)
        
        # ? »õ·Î¿î physics-aware loss °è»ê
        ...
        loss_breakdown.update({
            'loss_type': 'physics_aware_3_component',
    ...
    def get_model_analysis(self, sample_input: torch.Tensor) -> Dict:
        """¸ðµ¨ ÀüÃ¼ ºÐ¼® (Shape È£È¯¼º º¸Àå)"""
        self.eval()
        
        # ? ¿Ã¹Ù¸¥ ¹æ¹ý: get_features() »ç¿ë (ÀÌ¹Ì shape º¯È¯ Æ÷ÇÔ)
        if next(self.parameters()).device != sample_input.device:
            sample_input = sample_input.to(next(self.parameters()).device)
        
        with torch.no_grad():
            # ? Á¤»ó °æ·Î »ç¿ë
            features = self.get_features(sample_input)  # 5D ¡æ 4D º¯È¯ ÀÚµ¿ Ã³¸®
            
            # ? ÀÌ¹Ì º¯È¯µÈ 4D inputÀ¸·Î feature_extraction Á÷Á¢ È£ÃâÇÏÁö ¾Ê°í
            # get_features() ³»ºÎ¿¡¼­ Ã³¸®µÈ °á°ú È°¿ë
            feature_stats = {
                'input_info': {
