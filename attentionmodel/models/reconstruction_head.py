...
class MagnitudePhaseConverter:
    """Magnitude/Phase º¯È¯ À¯Æ¿¸®Æ¼ (reconstruction head¿ë)"""
    
    @staticmethod
    def magnitude_phase_to_real_imag(data: torch.Tensor) -> torch.Tensor:
        """°³¼±µÈ º¯È¯ - ¼öÄ¡Àû ¾ÈÁ¤¼º È®º¸"""
        magnitude = data[..., 0]
        phase = data[..., 1]
        
        # ? ¼öÄ¡Àû ¾ÈÁ¤¼ºÀ» À§ÇÑ ¹üÀ§ Á¦ÇÑ
        magnitude = torch.clamp(magnitude, min=1e-8, max=1e6)  # ³Ê¹« Å« °ª ¹æÁö
...
class EnhancedFrequencySpecificHead(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # ³×Æ®¿öÅ© ¿¹Ãø
        mag_phase_output = self.head(x)  # (batch, 361, 2)
        
        # ?? ¼öÁ¤: Phase Ã³¸® (±âÁ¸°ú µ¿ÀÏ)
        phase = mag_phase_output[..., 1]
        phase = torch.atan2(torch.sin(phase), torch.cos(phase))
        
        # ?? ¼öÁ¤: Magnitude Ã³¸® (ÇÙ½É º¯°æ!)
        magnitude = torch.abs(mag_phase_output[..., 0]) * 10.0 + 1e-8  # softplus ¡æ abs * scale
        
        # ?? ¼öÁ¤: ÁÖÆÄ¼öº° °¡ÁßÄ¡ (ÇÙ½É º¯°æ!)
        if self.frequency_weight is not None:
            weight = F.softplus(self.frequency_weight) + 0.5  # sigmoid ¡æ softplus + 0.5
            magnitude = magnitude * weight
        
        # ±âÁ¸ º¯È¯ ·ÎÁ÷ À¯Áö
        mag_phase_constrained = torch.stack([magnitude, phase], dim=-1)
        real_imag_output = MagnitudePhaseConverter.magnitude_phase_to_real_imag(mag_phase_constrained)
...
class FrequencySpecificReconstructionHead(nn.Module):
    ...
    def compute_enhanced_loss(self, reconstructed: torch.Tensor, original: torch.Tensor, 
                            mask: torch.Tensor, return_breakdown: bool = True) -> Tuple[torch.Tensor, Dict]:
        """
        ? ÇÙ½É ¼öÁ¤: Physics-aware loss »ç¿ë
        """
        # ? »õ·Î¿î physics-aware loss calculator »ç¿ë
        total_loss, loss_breakdown = self.physics_loss_calculator.compute_total_loss(
            reconstructed, original, mask, return_breakdown=return_breakdown
        )
