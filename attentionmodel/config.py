...
class EEGConfig:
    ...
    LOSS_CONFIG = {
        # ? 3-component balanced loss weights (±âÁ¸ 4°³ ¡æ 3°³ ´Ü¼øÈ­)
        'loss_weights': {
            'magnitude': 0.6,    # ¿¬°á °­µµ (°¡Àå Áß¿ä) - ±âÁ¸ 25% ¡æ 50%
            'phase': 0.3,        # ½Ã°£ Áö¿¬/À§»ó - ±âÁ¸ 35% ¡æ 30% 
            'coherence': 0.1     # EEG Æ¯È­ coherence - ±âÁ¸ 10% ¡æ 20%
            # 'mse' Á¦°Å (magnitude loss¿¡ Æ÷ÇÔµÊ)
        },
        
        # ? Enhanced Magnitude Loss ¼³Á¤ (±âÁ¸º¸´Ù ´Ü¼øÈ­)
        'magnitude_loss_config': {
            'loss_type': 'enhanced_l2',      # ±âÁ¸ 'l2' ¡æ 'enhanced_l2' 
            'relative_weight': 0.3,          # ±âÁ¸ 0.6 ¡æ 0.3 (»ó´ë¿ÀÂ÷ ºñÁß °¨¼Ò)
            'frequency_weight': 0.3,         # ÁÖÆÄ¼ö °¡ÁßÄ¡ ºñÁß
            'basic_weight': 0.4,             # ±âº» L2 loss ºñÁß
            
            # ÁÖÆÄ¼ö ´ë¿ªº° °¡ÁßÄ¡ (Alpha Áß½É ÀçÁ¶Á¤)
            'frequency_weights': {
                'delta': 1.0,     # ±âÁ¸ 0.5 ¡æ 1.0 (Áõ°¡)
                'theta': 1.2,     # ±âÁ¸ 1.3 ¡æ 1.2 (¾à°£ °¨¼Ò)
                'alpha': 1.8,     # ±âÁ¸ 1.8 À¯Áö (°¡Àå Áß¿ä)
                'beta1': 1.5,     # ±âÁ¸ 1.6 ¡æ 1.5 (¾à°£ °¨¼Ò)  
                'beta2': 1.3,     # ±âÁ¸ 1.4 ¡æ 1.3 (¾à°£ °¨¼Ò)
                'gamma': 1.1      # ±âÁ¸ 1.2 ¡æ 1.1 (¾à°£ °¨¼Ò)
            }
        },
        
        # ? Circular Phase Loss ¼³Á¤ (ÇÙ½É º¯°æ!)
        'phase_loss_config': {
            'loss_type': 'circular_l2',      # ? 'cosine' ¡æ 'circular_l2' (gradient ¾ÈÁ¤È­!)
            'wrap_around': True,             # Phase wrapping Ã³¸®
            'frequency_emphasis': 'alpha',   # Alpha ´ë¿ª °­Á¶
            'alpha_weight': 0.3,             # Alpha phase loss ºñÁß
            'basic_weight': 0.7              # ±âº» phase loss ºñÁß
            # 'temperature' Á¦°Å (circular_l2¿¡¼­ ºÒÇÊ¿ä)
        },
        
        # ? EEG Physics-Aware Coherence ¼³Á¤ (¿ÏÀü Àç¼³°è)
        'coherence_loss_config': {
            'coherence_type': 'eeg_physics_aware',  # »õ·Î¿î Å¸ÀÔ
            
            # 4°¡Áö EEG Æ¯È­ coherence ±¸¼º ¿ä¼Ò
            'component_weights': {
                'power_coherence': 0.4,      # ¿¬°á °­µµ ÀÏ°ü¼º
                'cross_frequency': 0.3,      # ÁÖÆÄ¼ö ´ë¿ª °£ °ü°è  
                'spatial_coherence': 0.2,    # ½ÇÁ¦ Àü±Ø ÀÎÁ¢¼º
                'hermitian_symmetry': 0.1    # ´ëÄª¼º º¸Á¸
            },
            
            # Cross-frequency coherence ¼³Á¤
            'cross_freq_pairs': [
                ('alpha', 'beta1'),          # Alpha-Beta °ü°è
                ('theta', 'alpha'),          # Theta-Alpha °ü°è
                ('beta1', 'beta2')           # Beta1-Beta2 °ü°è
            ],
            
            # ½ÇÁ¦ Àü±Ø ÀÎÁ¢¼º »ç¿ë ¿©ºÎ
            'use_real_electrode_adjacency': True,
            
            # Hermitian ´ëÄª¼º tolerance
            'hermitian_tolerance': 1e-3
            
            # ±âÁ¸ º¹ÀâÇÑ ¼³Á¤µé Á¦°Å:
            # - 'spatial_coherence_weight' Á¦°Å
            # - 'temporal_coherence_weight' Á¦°Å  
            # - 'coherence_type': 'magnitude_consistency' Á¦°Å
        }
    }
        # =============== Training Configuration (¾ÈÁ¤¼º ¿ì¼±) ===============
    TRAINING_CONFIG = {
        'batch_size': 256,             # 256 ¡æ 32 (¾ÈÁ¤¼º ¿ì¼±)
        'num_epochs': 50,
    ...
    PRETRAINING_CONFIG = {
        ...
        'num_epochs': 50,
        'learning_rate': 1e-4,        # 1e-3 ¡æ 5e-4 (¾ÈÁ¤¼º)
        'batch_size': 256,             # 256 ¡æ 32 (¾ÈÁ¤¼º)
        'weight_decay': 1e-4,         # 3e-3 ¡æ 2e-3
    ...
    DATA_CONFIG = {
        ...
        'checkpoint_path': '/home/mjkang/cbramod/attentionmodel/checkpoints/',
        'log_path': '/home/mjkang/cbramod/attentionmodel/logs/',
        'data_key': 'csd'
    }
    ...
    def validate_configuration(cls):
        ...
        assert phase_config['loss_type'] == 'circular_l2', "Should use circular_l2 loss for gradient stability and 90¡Æ problem resolution"
        
        print("? Enhanced CNN Configuration validation passed!")
        print(f"   ?? Phase loss: {phase_config['loss_type']} (90¡Æ problem solved)")
