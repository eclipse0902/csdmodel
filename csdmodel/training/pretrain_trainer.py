...
class EnhancedMaskingStrategy:
    ...
    ±âÁ¸ ´Ü¼ø random masking + ¼º´É ÃÖÀûÈ­
    4-5M ÆÄ¶ó¹ÌÅÍ ¸ðµ¨¿¡ ¸Â´Â ¸¶½ºÅ· Àü·«
    """
    ...
    def __init__(self, mask_ratio: float = 0.5, config: EEGConfig = None):
        ...
        # Enhanced masking options (Optional, ±âº»Àº ´Ü¼ø masking)
        self.use_adaptive_masking = getattr(config, 'ENHANCED_MASKING', {}).get('adaptive', False)
        
        print(f"?? Enhanced Masking Strategy:")
        print(f"   Mask ratio: {mask_ratio}")
        print(f"   Strategy: Random uniform masking (proven effective)")
    ...
    def apply_masking(self, data: torch.Tensor, epoch: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Args:
            data: (batch, 15, 19, 19, 2) - CSD data
            epoch: Current training epoch (for adaptive masking)
            
        Returns:
            masked_data: (batch, 15, 19, 19, 2)
            mask: (batch, 15, 19, 19, 2) - 1=keep, 0=mask
        """
        batch_size, freq, height, width, complex_dim = data.shape
        device = data.device
        
        # Adaptive mask ratio based on training progress (optional)
        current_mask_ratio = self.mask_ratio
        if self.use_adaptive_masking and epoch is not None:
            # Start with higher masking, gradually reduce
            progress = min(epoch / 50.0, 1.0)  # Assume 50 epochs max
            current_mask_ratio = self.mask_ratio * (1.2 - 0.2 * progress)
            current_mask_ratio = min(current_mask_ratio, 0.7)  # Cap at 70%
        
        # Create mask
        mask = torch.ones_like(data)
        
        # Enhanced random masking for each sample in batch
        for b in range(batch_size):
            # Total positions to mask
            total_positions = height * width  # 361
            num_to_mask = int(total_positions * current_mask_ratio)
...
class EEGPretrainTrainer:
    ...
    def train(self) -> Dict:
        ...
        print(f"   15 Frequency-Specific Heads")
...
if __name__ == "__main__":
    ...
    class MockDataset:
        ...
        def __getitem__(self, idx):
            return torch.randn(15, 19, 19, 2), torch.tensor([0])
    ...
    try:
        ...
        sample_data = torch.randn(2, 15, 19, 19, 2)
